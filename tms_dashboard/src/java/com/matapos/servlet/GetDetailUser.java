 ...
 * @author azzam
 */
@WebServlet(name = "GetDetailUser", urlPatterns = {"/GetDetailUser"})
public class GetDetailUser extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet GetDetailUser</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet GetDetailUser at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
...
//        MessageProcess mp = new MessageProcessImpl();
        ...
        try {
            ...
            String account_id = newJson.get("account_id").toString();
            String merchant_id = newJson.get("merchant_id").toString();
            req.put(FieldParameterMatapos.proccode, ProcessingCode.getdetailuser);
